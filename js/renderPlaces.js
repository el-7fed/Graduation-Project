...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Re-render places when language changes
...
function createPlaceCard(place) {
  ...
  const currentLang = window.languageUtils
    ...
    : "ar";
  const viewDetailsText = window.languageUtils
    ? window.languageUtils.t("viewDetails")
    : "View Details";
  const displayName =
  ...
  const categoryText = window.languageUtils
    ? window.languageUtils.t(categoryKey)
    : place.category;
  const displayCategory = (categoryText || place.category || "").toUpperCase();

  card.innerHTML = `
        <div class="place-card-image">
            <img src="${imageSrc}" alt="${displayName}" onerror="this.src='assets/images/placeholder.jpg'">
            <span class="place-card-category">${displayCategory}</span>
        </div>
        <div class="place-card-content">
            <h3 class="place-card-title">${displayName}</h3>
            <p class="place-card-location">📍 ${displayLocation}</p>
            ...
            <div class="place-card-footer">
                ...
                <a href="place-details.html?id=${
                  place.id
                }" class="btn btn-primary">${viewDetailsText}</a>
            </div>
        </div>
    `;
...
function filterAndRenderPlaces() {
  ...
  let filtered = placesData.filter((place) => {
    ...
    const descMatch =
      ...
      (place.descriptionEn &&
        place.descriptionEn.toLowerCase().includes(searchTerm));
    const locationMatch =
...
function updateResultsCount(count) {
  ...
  if (resultsInfo) {
    const currentLang = window.languageUtils
      ...
      : "ar";
    if (currentLang === "ar") {
      resultsInfo.textContent = `عرض ${count} مكان`;
    } else {
      resultsInfo.textContent = `Showing ${count} place${
        count !== 1 ? "s" : ""
      }`;
    }
  }
